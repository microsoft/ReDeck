"""CodeRepairWorker - repairs generated slide code.
...
class CodeRepairWorker:
    ...
    def repair_slides(
        ...
        # Only repair layout/visual issues (B*). Content (C*, D*, E*) and
        # narrative (A*) issues cause regressions when repaired via HTML rewrite.
        for sid in list(slide_issues.keys()):
            ...
            if not slide_issues[sid]:
                logger.info("Slide %d: skipping repair (no layout issues)", sid)
